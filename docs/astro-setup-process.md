...
## Phase 5 — Prune wrong keywords

Use MCP `remove_keywords` (destructive). Prune anything not in `scripts/astro-keywords-us.json`:

```bash
./scripts/astro-prune-keywords.sh
```

Also remove manually in Astro UI if needed:
...
| `remove_keywords` | Delete tracked keywords (max 100/batch) |
