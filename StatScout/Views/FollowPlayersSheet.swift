...
struct FollowPlayersSheet: View {
    ...
    init(viewModel: DashboardViewModel, side: TrendSide = .batting) {
    ...
    private var candidates: [Player] {
        ...
        let sameSide: [Player] = allPlayers.filter { player in
            (player.playerType ?? "batter") == wanted
        }
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                ...
                Image(systemName: following ? "star.fill" : "star")
                    ...
                    .foregroundStyle(following ? Color.yellow : SavantPalette.inkTertiary)
            }
