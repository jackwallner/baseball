...
- **Game types / phases**: `pybaseball.statcast()` requests `hfGT=R|PO|S`, so every date-range pull carries spring training and the postseason too. Spring is dropped; the postseason is written to `player_postseason_game_logs`, never to `player_game_logs`. That split is load bearing: shipped builds query `player_game_logs` with no phase filter of their own, so a playoff row there lands inside a regular-season "last 7 days". `STATCAST_GAME_TYPES` overrides the collected set for one-off runs. Savant publishes no postseason percentile leaderboards, so postseason can only ever cover game logs and standard stats, never percentile bars.
