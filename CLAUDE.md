...
- **Game types**: `pybaseball.statcast()` requests `hfGT=R|PO|S`, so every date-range pull carries spring training and the postseason too. `player_game_logs.game_type` records which, and `ingest_game_logs.py` writes regular season only (`STATCAST_GAME_TYPES` widens it) because the app queries that table directly with no phase filter of its own. Savant publishes no postseason percentile leaderboards, so postseason can only ever cover game logs and standard stats.
